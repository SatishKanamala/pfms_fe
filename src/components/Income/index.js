...
function Income() {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/transaction/income/get_all/?limit=${pageSize}&offset=${(currentPage - 1) * pageSize}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.data && response.data.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.id) {
        ...
        response = await axios.put(`http://localhost:8000/api/v1/transaction/income/update/${formData.id}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        response = await axios.post("http://localhost:8000/api/v1/transaction/income/create", formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/api/v1/transaction/income/delete/${deleteTransactionId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
